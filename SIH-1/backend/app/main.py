...
class OutputPrediction(BaseModel):
    prediction : str
    confidence: float  # Add confidence as a float percentage
...
def predict(request: InputSequence):
    ...
    # Calculate confidence as softmax probability of predicted class
    probabilities = torch.nn.functional.softmax(output, dim=1)
    confidence = probabilities[0][predicted_class].item() * 100  # percentage

    return {"prediction": predicted_label[0], "confidence": confidence}
